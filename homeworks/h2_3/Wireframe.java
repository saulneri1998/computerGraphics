...
public class Wireframe extends JFrame implements KeyListener, FocusListener{
    ...
    public Wireframe(Point3D[] vertices, Face[] faces) {
        setSize(1300, 700);
    ...
    class Canvas extends JPanel {
        public void paintComponent(Graphics g) {
            ...
            if (!focussed) {
                ...
                // Sort faces (bubblesort)
                for (int m = 0; m < faces.length; m++) {
                    for (int n = 0; n < faces.length-1; n++) {
                        if (faces[n].getDistance(vertices, theta, phi) < faces[n+1].getDistance(vertices, theta, phi)) {
                            //Swap
                            Face temp = faces[n];
                            faces[n] = faces[n+1];
                            faces[n+1] = temp;
                ...
                for ( j = 0; j < faces.length; ++j ) {
                    int[] xs = {  (int)Math.round(points[faces[j].p1].x), 
                                (int)Math.round(points[faces[j].p2].x), 
                                (int)Math.round(points[faces[j].p3].x), 
                                (int)Math.round(points[faces[j].p4].x)
                            };
                    int[] ys = {  (int)Math.round(points[faces[j].p1].y), 
                                (int)Math.round(points[faces[j].p2].y), 
                                (int)Math.round(points[faces[j].p3].y), 
                                (int)Math.round(points[faces[j].p4].y)
                            };
